...
driver.get("https://clairemont.sandiegounified.org/contacts/faculty__teachers")
...
for tr_element in tr_elements:
    ...
    if len(td_elements) <= 5:  
        ...
        try:
            ...
            teacher_details.append([name, email_address1,department,academy,school_name]) 
            print(teacher_details)
        # Add the teacher details to the list
        except:
            continue
               
with open('staff_data_clairemont.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile, delimiter=',')
    writer.writerow(['name','email_address','deprtment','academy','school_name'])
